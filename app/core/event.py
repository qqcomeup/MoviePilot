...
class EventManager(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        # 插件管理器确认后的事件处理器归属，key 为 handler_identifier，value 为 (plugin_id, handler_object_id)
        self.__handler_plugin_ids: Dict[str, Tuple[str, int]] = {}
        # 禁用的事件处理器集合
    ...
    def add_event_listener(self, event_type: Union[EventType, ChainEventType], handler: Callable,
        ...
        with self.__lock:
            ...
            self.__handler_plugin_ids.pop(handler_identifier, None)
    ...
    def remove_event_listener(self, event_type: Union[EventType, ChainEventType], handler: Callable):
        ...
        with self.__lock:
            ...
            if not self.__is_handler_registered(handler_identifier):
                self.__handler_plugin_ids.pop(handler_identifier, None)
    ...
    def bind_plugin_event_handlers(self, plugin_id: str, plugin_cls: type):
        ...
        with self.__lock:
            self.__handler_plugin_ids = {
                handler_id: binding
                for handler_id, binding in self.__handler_plugin_ids.items()
                if binding[0] != plugin_id
            }
    ...
    @classmethod
    def __get_plugin_handler_object_ids(cls, plugin_cls: type) -> set:
        """
        获取插件类及其基类上的可调用处理器对象ID。
        """
        handler_ids = set()
        for candidate_cls in inspect.getmro(plugin_cls):
            if candidate_cls is object:
                continue
            for value in candidate_cls.__dict__.values():
                handler = cls.__normalize_handler_callable(value)
                if callable(handler):
                    handler_ids.add(id(handler))
        return handler_ids

    def __bind_subscriber_handlers(self, plugin_id: str, plugin_handler_ids: set, subscribers: Dict[str, Any]):
        """
        ...
        for handler_identifier, handler_data in subscribers.items():
            ...
            if handler_object_id in plugin_handler_ids:
                self.__handler_plugin_ids[handler_identifier] = (plugin_id, handler_object_id)
    ...
    def __bind_loaded_plugin_event_handler(self, handler_identifier: str, handler: Callable):
        ...
        """
        handler_object_id = id(self.__normalize_handler_callable(handler))
        class_name, _ = self.__parse_handler_names(handler)
        if not class_name:
            return
        try:
            from app.core.plugin import PluginManager
        ...
        if (
                ...
                and handler_object_id in self.__get_plugin_handler_object_ids(plugin_cls)
        ):
            with self.__lock:
                if self.__is_handler_object_registered(handler_identifier, handler_object_id):
                    self.__handler_plugin_ids[handler_identifier] = (class_name, handler_object_id)
    ...
    def __is_handler_object_registered(self, handler_identifier: str, handler_object_id: int) -> bool:
        ...
        for subscribers in self.__broadcast_subscribers.values():
            ...
            if handler is not None and id(self.__normalize_handler_callable(handler)) == handler_object_id:
                return True
        for subscribers in self.__chain_subscribers.values():
            handler_data = subscribers.get(handler_identifier)
            if handler_data is not None and id(self.__normalize_handler_callable(handler_data[1])) == handler_object_id:
                return True
        return False
    ...
    def __should_dispatch_to_target_plugin(
        ...
        if identifier_parts[-2:] != [class_name, method_name]:
            ...
            return False
        binding = self.__handler_plugin_ids.get(handler_identifier)
        handler_object_id = id(self.__normalize_handler_callable(handler))
        if not binding or binding != (target_plugin_id, handler_object_id):
            logger.debug(
        ...
        if expected_class != target_plugin_id:
            ...
            return False
        if handler_object_id not in self.__get_plugin_handler_object_ids(plugin_cls):
            logger.debug(
